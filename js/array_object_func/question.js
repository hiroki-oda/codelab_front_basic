...
console.log(cars.length + cars[1].length - 1); //6
...
function multipleedCalc(a, b) {
  return a * b;
}
console.log(multipleedCalc(5, 2));
...
// 理由：最初のconsole.log(getFruitName());の方はグローバル変数を対象としているので関数内でのfruitの値となる。つまりバナナ。次のconsole.log(fruit);はグローバル変数でプログラムの全範囲がfruitの対象となる。よってりんごとなる。
var fruit = 'りんご';
...
// 理由：今回はvarを使った変数とは違い、＝つまり代入でいれているのでcountryにアメリカが代入になった時点でアメリカのままであるから。
country = '日本';
...
// 理由:グローバルスコープか関数スコープの変数しかない。モジュールベースではない。厳格モードへの切り替えが単なる文字列に過ぎない。という理由。
(function() {
  var shop = 'コンビニ';
  console.log(shop)
}());
